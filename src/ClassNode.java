...
public class ClassNode implements Node, DecNode {
	...
	public Node typeCheck() {
		...
		// verifico che effettivamente le espressioni definite nel metodo ritornino il tipo di ritorno dichiarato
		...
		if(this.superEntry!=null){
			...
			for(int i=0;i<superFields.size();i++){
				...
				if(!FOOLLib.isSubType(superField.getSymType(),classField.getSymType())){
					System.out.println("Wrong type for "+i+"-th field in "+this.type.getType());
					System.exit(0);
			...
			// per quanto riguarda i metodi è già tutto implementato in FOOLLib dalla gestione dell'high order
